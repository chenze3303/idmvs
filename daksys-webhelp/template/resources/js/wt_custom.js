...
// "必须显示"属性名称
var FILTER_DISPLAY = "FILTERDISPLAY";
// 必须显示项目 -- zTree 使用
...
// 1)页面[MODELSWITCH]开关判断, 2)读取json文件判断
...
$(document).ready(function () {
	...
	// DEBUG start
	// localStorage.removeItem("MANUALID_TYPE_ATTR_DS-1600K=GUID-1EB3D536-6878-48CD-A823-4622932E60F5");
...
function displaySelectModel(){
	...
	var model_onoff = $( "input[name$='MODELSWITCH']" ).val();
	if(model_onoff == SWITCH_HIDE){
		console.log("MODELSWITCH>>>隐藏型号列表");
		$("#div_model").remove();
...
function getHiddenInfoTypeName(){
	// <input type="hidden" name="FUNCTION" value="功能" />
...
function setCustomModeleAttrs(){
	//从搜索页面跳转 --- 设置页面选中项目
	var selected_model = localStorage.getItem(STORAGE_MANUALID_MODEL);
	...
	if(HTML_LANG == 'en-US'){
		$('#div_custom_type_name_en input').each(function(){
			var attrType = $(this).attr("name");
			//var attrName = $(this).attr("value");
			var attrValString = localStorage.getItem(STORAGE_MANUALID_TYPE_ATTR + "," + attrType);
...
function btnConfirmCustom(){
	...
	if(!hasCondition) {
		...
		webAlert(getLocalization("Tip"), alertText, function(){   
			//要回调的方法 
		}); 
...
function execCustomMenu(){
	...
	// 型号
	// STORAGE 清除
	localStorage.removeItem(STORAGE_MANUALID_MODEL);
	...
	if(DISPLAY_MODEL_SELECT){
		...
		if(model_val != ""){
			...
			// -------------------------------
			localStorage.setItem(STORAGE_MANUALID_MODEL, model_val);
	...
	KEY_TYPE_NAME_MAP.forEach((attrName, attrType)=>{
		// STORAGE 清除
		localStorage.removeItem(STORAGE_MANUALID_TYPE_ATTR + "," + attrType);
		...
		// -------------------------------
		//console.log("STORAGE key >>>" , STORAGE_MANUALID_TYPE_ATTR + "," + attrType);
		//console.log("STORAGE content >>>" , attrValArray.toString());
		localStorage.setItem(STORAGE_MANUALID_TYPE_ATTR + "," + attrType, attrValArray.toString());
	});
	// 过滤逻辑
	// 根据 zTree 右树过滤 menu 
	var zTreeRight = $.fn.zTree.getZTreeObj("treeRight");
	...
	if(g_guidArray.length > 0){
		// 保存 topic guid
		//console.log("STORAGE key >>>" , STORAGE_MANUALID_TOPIC);
		localStorage.setItem(STORAGE_MANUALID_TOPIC, g_guidArray.toString());
	}
	// 定制状态: TRUE
	IS_MENU_CUSTOM = true;
...
function btnResetCustom(){
	...
	webConfirm(getLocalization("Tip"), confirmText, function(data){   
		//要回调的方法  
		if(data){
...
function cleanLocalStorage(){
	...
	localStorage.removeItem(STORAGE_MANUALID_MODEL);
	localStorage.removeItem(STORAGE_MANUALID_TOPIC);
	//属性
	KEY_TYPE_NAME_MAP.forEach((attrName, attrType)=>{
		localStorage.removeItem(STORAGE_MANUALID_TYPE_ATTR + ',' + attrType);
	});
