	...
	if (isMobile()) {
		...
		if (!this.resizeDebounce) {
			...
			if ($(window).scrollTop() > $(window).height() * 0.75) {
				if (!backToTopVisible) {
					$('.body_to_top').fadeIn(200);
					backToTopVisible = true;
				}
			} else {
				if (backToTopVisible) {
					$('.body_to_top').fadeOut(200);
...
// 显示/隐藏置顶按钮 - 移动端优化版本
    if (isMobile()) {
        // 使用防抖优化性能，避免频繁触发
        if (!this.debounceBodyToTop) {
            this.debounceBodyToTop = true;
            setTimeout(() => {
                this.debounceBodyToTop = false;
            }, 100);
            ...
            if ($(window).scrollTop() > $(window).height() * 0.75) {
                ...
                // 清除之前的隐藏定时器
                if (backToTopHideTimeout) {
                    clearTimeout(backToTopHideTimeout);
                    backToTopHideTimeout = null;
                }
            } else {
                // 隐藏按钮
                if (backToTopVisible) {
        ...
        // 滚动时清除隐藏定时器，滚动停止后开始计时隐藏
        if (backToTopVisible) {
            if (backToTopHideTimeout) {
                clearTimeout(backToTopHideTimeout);
            }
            // 1.5秒后隐藏按钮
            backToTopHideTimeout = setTimeout(function() {
                if (backToTopVisible && $(window).scrollTop() > $(window).height() * 0.75) {
                    $('.body_to_top').fadeOut(200);
                    backToTopVisible = false;
                }
            }, 1500);
