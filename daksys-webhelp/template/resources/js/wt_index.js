...
var STORAGE_MANUALID_TYPE_ATTR = "";
// STORAGE 手册名称ID + 定制的型号
var STORAGE_MANUALID_MODEL = "";
// STORAGE 手册名称ID + 章节
var STORAGE_MANUALID_TOPIC = "";
...
$(document).ready(function () {
	...
	// STORAGE key
	STORAGE_MANUALID_TYPE_ATTR = 'MANUALID_TYPE_ATTR_' + $('#document-id').text();
	STORAGE_MANUALID_MODEL = "MANUALID_MODEL_" + $('#document-id').text();
	STORAGE_MANUALID_TOPIC = "MANUALID_TOPIC_" + $('#document-id').text();
	// lang
	...
	$("#div_custom_win").attr('style','z-index: -999;');
 	...
 	if(!isOffLine()){
		...
		console.log('●●●●● online');
		$('.wt_header_langue_mobile').show();
		if (!isMobile()) {
		$('.wt_header_langue').show();
		$('.wt_header_QRcode').show();
		}
	} else {
		// 离线状态
		if(!isMobile()){
	...
	var storage_guidString = localStorage.getItem(STORAGE_MANUALID_TOPIC);
