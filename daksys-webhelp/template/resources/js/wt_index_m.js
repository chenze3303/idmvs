...
$(document).ready(function () {
	...
	// 监听 iframe 内的滚动事件，实现本页导航同步
	setInterval(checkIframeScroll, 100);
...
function openTopicAnchorList() {
	if ($('#frame_element').contents().find('.right_guide').find('li').length > 0) {
        ...
        // 250822新增h4h5层级对应
		} else if ($li.hasClass('feature_guide_h4')) {
			anchor_item.addClass('wt_anchor_h4');
		} else if ($li.hasClass('feature_guide_h5')) {
			anchor_item.addClass('wt_anchor_h5');
		} else {
...
// 检查iframe滚动位置，同步本页导航选中状态
function checkIframeScroll() {
    try {
        var frameElement = document.getElementById("frame_element");
        ...
        // 获取iframe内的滚动位置
        var scrollTop = frameWindow.scrollY || frameWindow.pageYOffset || 0;
        
        // 查找所有可导航的标题元素
        var titleElements = [];
        $frame.find('h1, h2, h3, h4, h5').each(function() {
            var $el = $(this);
            var id = $el.attr('id') || $el.parent().attr('id');
            if (id) {
                var offsetTop = 0;
                try {
        ...
        // 按位置排序
        titleElements.sort(function(a, b) {
            return a.top - b.top;
        });
        ...
        for (var i = titleElements.length - 1; i >= 0; i--) {
            if (titleElements[i].top <= scrollTop + 100) { // 100px的偏移量，让用户看到部分内容就算作当前章节
                currentId = titleElements[i].id;
                break;
            }
        }
        ...
        if (currentId) {
            // 检查是否需要更新（避免频繁DOM操作）
            var $currentActive = $('.wt_footer_mobile_anchor_list .wt_anchor_item.active');
            if ($currentActive.length === 0 || $currentActive.attr('data-id') !== currentId) {
                // 移除所有active状态
                $('.wt_footer_mobile_anchor_list .wt_anchor_item').removeClass('active');
                // 添加当前active状态
                $('.wt_footer_mobile_anchor_list .wt_anchor_item[data-id="' + currentId + '"]').addClass('active');
                
                // 平滑滚动到选中的导航项
                var $activeItem = $('.wt_footer_mobile_anchor_list .wt_anchor_item[data-id="' + currentId + '"]');
                if ($activeItem.length > 0) {
                    var itemTop = $activeItem.position().top;
                    var itemHeight = $activeItem.outerHeight();
                    var listHeight = $('.wt_footer_mobile_anchor_list').height();
                    var currentScrollTop = $('.wt_footer_mobile_anchor_list').scrollTop();
                    
                    // 计算目标滚动位置，使选中项居中显示
                    var targetScrollTop = currentScrollTop + itemTop - listHeight/2 + itemHeight/2;
                    
                    // 平滑滚动到目标位置
                    $('.wt_footer_mobile_anchor_list').stop().animate({
        ...
        // 忽略跨域错误或其他异常
        // console.log('checkIframeScroll error:', e);
	}
}
